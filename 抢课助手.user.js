...
// @version      1.1.5.5
// @description  自动抢课，支持队列持久化，自动确认弹窗、自动确认默认关闭
    ...
    $.ajax({
        ...
        success: function(n) {
            if (n.version !== pro_info.version) {
                var e = $("h1");
                e.html(e.html() + `<h5 style="font-size: 16px;display: inline;margin: 0">(检查到新版本v${n.version}：${n.newVersionDescription}[<a href='https://github.com/TLSecrets/ZFCourseTakingAssistant/raw/main/%E6%8A%A2%E8%AF%BE%E5%8A%A9%E6%89%8B.user.js'>点此更新</a>])</h5>`);
            }
        }
    });

    setTimeout(run, 100);
};window.addOptionListener=function(n){cur=$(n).attr("id"),$("#del").css("display","block").click((function(){$("#"+cur).remove(),delete take[cur],$("#del").css("display","none")}))}})()})();

// ========== Part B: Runtime Security Fixes ==========
(function(){
    window.zfc_escapeHtml = function(str){
        if(!str&&str!==0)return'';
        var s=String(str);
    ...
    $.fn.append = function(){
        ...
        for(var i=0;i<arguments.length;i++){
            ...
            if(typeof arg === 'string'){
                arg = arg.replace(/(value=')([^']*?)(')/g, function(m, p1, p2, p3){
                    return p1 + p2.replace(/'/g, '&#39;') + p3;
                });
            }
            args.push(arg);
        }
        return _origAppend.apply(this, args);
    };
    console.log('[抢课助手] 安全补丁已注入');
})();
window.zfc_safeTrim = function(str) {
    if (str === null || str === undefined) return '';
    if (typeof str !== 'string') str = String(str);
    return str.trim();
};
window.zfc_safeVal = function(selector, isJquery) {
    try {
        var el = isJquery ? $(selector) : document.querySelector(selector);
        if (!el) return '';
        var v = isJquery ? el.val() : el.value;
        return (v === null || v === undefined) ? '' : String(v);
    } catch(e) {
        return '';
    }
};
window.zfc_safeGetEl = function(id) {
    return document.getElementById(id) || null;
};

// ========== Queue Fix & Persistence ==========
(function() {
    function saveQueue() {
        try {
            localStorage.setItem('zfc_queue_backup', JSON.stringify(window.take || {}));
        } catch(e) {}
    }

    function restoreQueue() {
        try {
            var raw = localStorage.getItem('zfc_queue_backup');
            if (!raw) return;
            var obj = JSON.parse(raw);
            if (Object.keys(obj).length === 0) return;
            if (!window.fucking_data_courses || Object.keys(window.fucking_data_courses).length === 0) {
                setTimeout(restoreQueue, 500);
                return;
            }
            var valid = false;
            for (var key in obj) {
                ...
                var meta = obj[key];
                if (!meta || !meta.m_jxb_id) continue;
                var kch = window.fucking_map[meta.m_jxb_id];
                if (kch && window.fucking_data_courses[kch] && window.fucking_data_courses[kch][meta.m_jxb_id]) {
                    valid = true;
                } else {
                    delete obj[key];
                }
            ...
            if (Object.keys(obj).length === 0) {
                localStorage.removeItem('zfc_queue_backup');
                return;
            }
            window.take = obj;
            ...
            for (var key in obj) {
                if (!obj.hasOwnProperty(key)) continue;
                var meta = obj[key];
                var courseInfo = window.fucking_data_courses[window.fucking_map[meta.m_jxb_id]][meta.m_jxb_id];
                var className = courseInfo ? courseInfo.class_name : meta.m_jxb_id;
                var opt = document.createElement('option');
                opt.id = key;
                opt.value = meta.m_jxb_id;
                opt.textContent = className;
                opt.setAttribute('onclick', 'addOptionListener(this)');
                sel.appendChild(opt);
            }
            saveQueue();
        } catch(e) { console.error('恢复队列失败', e); }
    }

    var origAddListener = window.addOptionListener;
    window.addOptionListener = function(el) {
        if (!el) return;
        var jxbId = el.value;
        if (!jxbId) {
            var id = $(el).attr('id');
            if (id && window.take[id] && window.take[id].m_jxb_id) {
                jxbId = window.take[id].m_jxb_id;
            }
        }
        if (!jxbId) return;
        var course = window.fucking_data_courses && window.fucking_map[jxbId] ?
            window.fucking_data_courses[window.fucking_map[jxbId]][jxbId] : null;
        if (!course) return;
        var infDiv = $('#inf div');
        if (!infDiv.length) return;
        var con = '<style>span{display: block;font-size:12px;}</style>';
        var meta = null;
        for (var key in window.take) {
            if (window.take[key].m_jxb_id === jxbId) {
                meta = window.take[key];
                break;
            }
        }
        if (!meta) {
            con += '<button class="addd" data-jxbid="' + jxbId + '" style="background:#60BF80;color:#fff;border:none;padding:5px 10px;cursor:pointer;margin-bottom:5px;">添加到队列</button> ';
        }
        con += '<button class="del-from-queue" data-jxbid="' + jxbId + '" style="background:#EF6080;color:#fff;border:none;padding:5px 10px;cursor:pointer;margin-bottom:5px;">从队列移除</button><br>';
        con += '<span>教学班名称: ' + (course.class_name || '-') + '</span>';
        con += '<span>老师: ' + (course.jsxx || '-') + '</span>';
        con += '<span>当前人数: ' + (course.jxbrs || '-') + '</span>';
        con += '<span>容量: ' + (course.jxbrl || '-') + '</span>';
        con += '<span>教室: ' + (course.jxdd || '-') + '</span>';
        con += '<span>类别: ' + (course.kcgsmc || '-') + '</span>';
        con += '<span>课程类型: ' + (course.kcxzmc || '-') + '</span>';
        con += '<span style="font-weight: bold">上课时间: ' + (course.sksj || '-') + '</span>';
        con += '<span>校区: ' + (course.xqumc || '-') + '</span>';
        con += '<span>jxb_id: ' + jxbId + '</span>';
        if (course.meta) {
            con += '<span>---课程内部信息---</span>';
            con += '<span>do_jxb_id: ' + (course.meta.m_do_jxb_id || '-') + '</span>';
            con += '<span>kch_id: ' + (course.meta.kch_id || '-') + '</span>';
            con += '<span>jxbzls: ' + (course.meta.m_jxbzls || '-') + '</span>';
            con += '<span>选课类型: ' + (course.meta.course_type || '-') + '</span>';
        }
        infDiv.html(con);

        $('.addd').off('click').on('click', function() {
            var id = $(this).data('jxbid');
            var meta = null;
            var kch = window.fucking_map[id];
            if (kch && window.fucking_data_courses[kch] && window.fucking_data_courses[kch][id]) {
                meta = window.fucking_data_courses[kch][id].meta;
            }
            if (meta && !window.take.hasOwnProperty(meta.m_jxb_id)) {
                var newKey = '_' + new Date().getTime();
                window.take[newKey] = meta;
                var sel = document.getElementById('selected');
                var opt = document.createElement('option');
                opt.id = newKey;
                opt.value = meta.m_jxb_id;
                opt.textContent = window.fucking_data_courses[window.fucking_map[meta.m_jxb_id]][meta.m_jxb_id].class_name;
                opt.setAttribute('onclick', 'addOptionListener(this)');
                sel.appendChild(opt);
                saveQueue();
            }
        ...
        $('.del-from-queue').off('click').on('click', function() {
            var id = $(this).data('jxbid');
            var keyToRemove = null;
            for (var key in window.take) {
                if (window.take[key].m_jxb_id === id) {
                    keyToRemove = key;
                    break;
                }
            }
            if (keyToRemove) {
                delete window.take[keyToRemove];
                $('#' + keyToRemove).remove();
                saveQueue();
                $('#inf div').html('');
            }
    ...
    if (document.readyState === 'complete') {
        ...
        window.addEventListener('load', initQueuePersistence);
    }
})();
...
// ============================================================
(function() {
    window._autoConfirmEnabled = false;   // 全局变量，默认关闭

    function clickModalConfirm() {
        if (!window._autoConfirmEnabled) return;
        var modals = document.querySelectorAll('.modal.in, .modal.show');
        for (var i = 0; i < modals.length; i++) {
            var modal = modals[i];
            var btn = modal.querySelector('button[data-bb-handler="confirm"], button[data-bb-handler="ok"]');
            if (!btn) {
                btn = modal.querySelector('.btn-primary, .btn-confirm');
            }
            if (!btn) {
                var allBtns = modal.querySelectorAll('button');
                for (var j = 0; j < allBtns.length; j++) {
                    var text = allBtns[j].textContent.trim();
                    if (text.indexOf('确定') !== -1 || text.indexOf('确认') !== -1 || text.toUpperCase() === 'OK') {
                        btn = allBtns[j];
                        break;
                    }
                }
            ...
            if (btn && btn.offsetParent !== null) {
                btn.click();
                console.log('[抢课助手] 自动点击按钮:', btn.textContent.trim());
                return;
            }
        }
    }
        ...
        for (var i = 0; i < mutations.length; i++) {
            for (var j = 0; j < mutations[i].addedNodes.length; j++) {
                ...
                if (node.nodeType === 1 && node.classList && 
                    (node.classList.contains('modal') || node.querySelector && node.querySelector('.modal'))) {
                    setTimeout(clickModalConfirm, 100);
                }
            }
        }
    });
    ...
    window.toggleAutoConfirm = function(enable) {
        window._autoConfirmEnabled = enable;
...
(function() {
    function addToggleButton() {
        // 防止重复添加
        if (document.getElementById('auto-confirm-toggle')) return;
        var btnDomain = document.querySelector('.button_domain');
        if (!btnDomain) {
            setTimeout(addToggleButton, 500);
            return;
        }
        var toggleBtn = document.createElement('button');
        toggleBtn.id = 'auto-confirm-toggle';
        toggleBtn.style.cssText = 'background:#2a3a8a;color:#fff;border:1px white solid;cursor:pointer;display:block;margin:5px;width:100px;height:50px;';
        toggleBtn.textContent = '自动确认: 关闭';
        btnDomain.appendChild(toggleBtn);

        toggleBtn.addEventListener('click', function() {
            var isEnabled = window._autoConfirmEnabled || false;
            if (!isEnabled) {
                x0p({
                    title: '⚠️ 风险警告',
                    text: '开启“自动确认”后，所有弹窗（包括退课、删除等危险操作）将被自动点击确认。\n\n由此造成的任何后果，由使用者自行承担。\n\n确认开启？',
                    type: 'warning',
                    buttons: [
                        { type: 'cancel', text: '取消' },
                        { type: 'ok', text: '我已知晓风险，开启', key: 13, default: true }
                    ]
                }).then(function(result) {
                    if (result.button === 'ok') {
                        window.toggleAutoConfirm(true);
                        var btn = document.getElementById('auto-confirm-toggle');
                        if (btn) {
                            btn.textContent = '自动确认: 开启';
                            btn.style.background = '#8a2a2a';
                        }
                        x0p({ title: '已开启', text: '自动确认已启用，请谨慎操作。', type: 'info' });
                    }
                });
            } else {
                window.toggleAutoConfirm(false);
                var btn = document.getElementById('auto-confirm-toggle');
                if (btn) {
                    btn.textContent = '自动确认: 关闭';
                    btn.style.background = '#2a3a8a';
                }
                x0p({ title: '已关闭', text: '自动确认已禁用。', type: 'info' });
            }
        });
    }
